"use strict";var _interopRequireWildcard=require("@babel/runtime/helpers/interopRequireWildcard"),_interopRequireDefault=require("@babel/runtime/helpers/interopRequireDefault"),_defineProperty2=_interopRequireDefault(require("@babel/runtime/helpers/defineProperty")),_ava=_interopRequireDefault(require("ava")),_sinon=_interopRequireDefault(require("sinon")),_proxyquire=_interopRequireDefault(require("proxyquire")),_rxjs=require("rxjs"),_operators=require("rxjs/operators"),_rpcMessenger=require("@aragon/rpc-messenger"),apps=_interopRequireWildcard(require("./apps")),configurationKeys=_interopRequireWildcard(require("./configuration/keys")),apm=_interopRequireWildcard(require("./core/apm")),_utils=require("./utils"),_AsyncRequestCache=_interopRequireDefault(require("./utils/AsyncRequestCache")),callscriptUtils=_interopRequireWildcard(require("./utils/callscript")),forwardingUtils=_interopRequireWildcard(require("./utils/forwarding")),transactionsUtils=_interopRequireWildcard(require("./utils/transactions"));function ownKeys(object,enumerableOnly){var keys=Object.keys(object);if(Object.getOwnPropertySymbols){var symbols=Object.getOwnPropertySymbols(object);enumerableOnly&&(symbols=symbols.filter(function(sym){return Object.getOwnPropertyDescriptor(object,sym).enumerable})),keys.push.apply(keys,symbols)}return keys}function _objectSpread(target){for(var source,i=1;i<arguments.length;i++)source=null==arguments[i]?{}:arguments[i],i%2?ownKeys(Object(source),!0// force cache invalidation
...
const instance=createAragon("0x00",{cache:{forceLocalStorage:!0},events:{subscriptionEventDelay:1e3}});// assert
...
//   roles: []
// }],
proxyAddress:"0x789"}]),instance.calculateTransactionPath(null,"0x789").catch(err=>{t.is(err.message,"No functions specified in artifact for 0x789")})}),(0,_ava.default)("should use normal transaction pathing when finding external transaction path for installed app",async t=>{const{createAragon}=t.context,targetAddress="0x123",targetMethodJsonDescription=[{name:"foo"}],targetParams=[8],mockPath=[{to:"0x123",data:"0x456"}];t.plan(2);// arrange
...
t.deepEqual(externalPath,mockPath),t.true(instance.getTransactionPath.calledOnceWith(targetAddress,targetMethodJsonDescription.name,targetParams))}),(0,_ava.default)("should be able to find external transaction path for non-installed app",async t=>{const{createAragon,utilsStub}=t.context,targetAddress="0x123",mockTransaction={to:targetAddress,data:"0x123"};t.plan(1);// arrange
...
t.deepEqual(externalPath,[mockTransaction])}),(0,_ava.default)("should be able to find external transaction path for ACL",async t=>{const{createAragon,utilsStub}=t.context,targetAddress="0x123",targetMethodJsonDescription=[{name:"foo"}],targetParams=[8],mockPath=[{to:"0x123",data:"0x123"}];t.plan(2);// arrange
...
const externalPath=await instance.getExternalTransactionPath(targetAddress,targetMethodJsonDescription,targetParams);// assert
t.deepEqual(externalPath,mockPath),t.true(instance.getACLTransactionPath.calledOnceWith(targetMethodJsonDescription.name,targetParams))}),(0,_ava.default)("should run the app and reply to a request",async t=>{const{createAragon,messengerConstructorStub,utilsStub}=t.context;// Note: This is not a "real" unit test because the rpc handlers are not mocked
t.plan(4);// arrange
