...
let isForwarderOverride={};return app.isForwarder&&appInfo&&Array.isArray(appInfo.abi)&&!appInfo.abi.some(({type,name})=>"function"===type&&"isForwarder"===name)&&(isForwarderOverride={isForwarder:!1}),_objectSpread(_objectSpread(_objectSpread({},appInfo),app),isForwarderOverride)}))));this.apps=appsWithInfo$.pipe((0,_operators.publishReplay)(1)),this.apps.connect();/*******************************
     *                             *
     *            REPOS            *
     *                             *
...
(0,_operators.concatMap)(async([newRepoAppIds,updatedRepoAppIds])=>{const newRepos=(await Promise.all(newRepoAppIds.map(async appId=>{let repoProxy;try{const repoAddress=await this.ens.resolve(appId);repoProxy=(0,_repo.makeRepoProxy)(repoAddress,this.web3),await repoProxy.updateInitializationBlock()}catch(err){console.error(`Could not find repo for ${appId}`,err)}return{appId,repoProxy}}))).// Filter out repos we couldn't create proxies for (they were likely due to publishing
...
// full repo objects.
(0,_operators.concatMap)(async({repos,updatedRepoAppId:appId})=>{const{repoAddress,versions}=repos[appId],installedRepoInfo=installedRepoCache.get(appId),baseRepoInfo={appId,repoAddress,versions},fetchVersionInfo=version=>applicationInfoCache.request(`${appId}.${version.contractAddress}`).catch(()=>({})).then(content=>({content,version:version.version})),latestVersion=versions[versions.length-1],currentVersion=Array.from(versions)// Apply reverse to find the latest version with the currently installed contract address
...
return this.identityProviderRegistrar.has("local")?new Promise((resolve,reject)=>{this.identityIntents.next({address,providerName:"local",resolve,reject(err){reject(err||new Error("The identity modification was not completed"))}})}):Promise.reject(new Error(`Provider (${"local"}) not installed`))}/**
   ...
   *
   * @return {Promise<void>}
   */async initNetwork(){this.network=new _rxjs.ReplaySubject(1),this.network.next({id:await this.web3.eth.getChainId(),type:await this.web3.eth.net.getNetworkType()})}/**
...
return{shutdown,shutdownAndClearCache:async()=>(shutdown(),Object.keys(await this.cache.getAll()).reduce((promise,cacheKey)=>promise.then(()=>cacheKey.startsWith(proxyAddress)?this.cache.remove(cacheKey):Promise.resolve()),Promise.resolve()))}}}/**
   ...
   *   single transaction.
   */async getExternalTransactionPath(destination,methodJsonDescription,params){if((0,_utils.addressesEqual)(destination,this.aclProxy.address))try{return this.getACLTransactionPath(methodJsonDescription.name,params)}catch(_){return[]}const installedApp=await this.getApp(destination);if(installedApp)// Destination is an installed app; need to go through normal transaction pathing
return this.getTransactionPath(destination,methodJsonDescription.name,params);// Destination is not an installed app on this org, just create a direct transaction
// with the first account
const account=(await this.getAccounts())[0];try{const tx=await(0,_transactions.createDirectTransaction)(account,destination,methodJsonDescription,params,this.web3);return this.describeTransactionPath([tx])}catch(_){return[]}}/**
...
const app=await this.getApp(destination);if(!app)throw new Error(`Transaction path destination (${destination}) is not an installed app`);const methods=app.functions;if(!methods)throw new Error(`No functions specified in artifact for ${destination}`);// Find the relevant method information
